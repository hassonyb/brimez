...
local function instagramMedia(msg, query)
	...
	if jdat.data.caption then
	      ...
	      text = text.."▫️ the user : "..data.from.username:upper().."\n\n"
		  text = text..data.from.full_name.."\n\n"
		  text = text..data.text.."\n\n"
		  text = text.."▫️ number of like: "..jdat.data.likes.count.."\n"
    else
	      text = text.."▫️ the user : "..jdat.data.user.username:upper().."\n"
		  text = text.."▫️ the name : "..jdat.data.user.full_name.."\n"
		  text = text.."▫️ number of like: "..jdat.data.likes.count.."\n"
	end
	text = text..'▪️   The Sudo > @HHHHH6'
	send_msg(receiver,text,ok_cb,false)
end
